...
| 数据源 | 负责 | 获取方式 |
|---|---|---|
| **Liquipedia** | 赛历骨架：全年一级赛事的起止日期、奖金、场馆、冠亚军 | 本地 Node 代理（无 CORS，浏览器不能直连） |
| **5EPlay** | 官方中文赛事名、逐场赛程与比分、逐图比分、参赛战队（含队标与世界排名）、名次与奖金分配 | **浏览器直连**（返回 `Access-Control-Allow-Origin: *`，无需代理） |
| 内置快照 | 兜底：`data/tournaments.json` | 本地文件 |

这个分工带来一个实际好处：**GitHub Pages 上的托管版虽然跑不了 Node 代理，但 5EPlay 的赛程与战队数据照样能实时取到。**

### 怎么用

展开任意赛事卡片 → 三个分页：

- **赛程** —— 逐场对阵，含时间、轮次（如「淘汰赛 · 决赛」）、Bo 制、系列赛比分、逐图比分（标注每图胜方）
- **参赛战队** —— 队标墙 + 队伍名 + 世界排名（未开赛的赛事也能看到名单）
- **名次与奖金** —— 已结束的赛事显示最终名次与奖金；未结束的显示「奖金分配方案」并明确标注名次待定

只在你点开时才发起请求，结果在会话内缓存 10 分钟。

### 赛事 id 映射

Liquipedia 的赛事与 5EPlay 的 `tt_id` 需要对齐，映射表在 `data/5eplay-map.json`，由工具生成：

```bash
node tools/resolve-5eplay.js
```

匹配用三道硬门槛（日期重叠 / 赛事系列关键词 / 城市或名称证据）+ 全局贪心去重（防止两个本地赛事抢同一个 `tt_id`），每条都记录匹配依据便于人工复核。当前 36 个本地赛事命中 34 个——XSE 职业联赛和电竞国家杯在 5EPlay 上没有对应条目。

### 为什么 Liquipedia 需要 Node 代理

Liquipedia 的 `api.php` **不返回 `Access-Control-Allow-Origin`**，浏览器直连会被 CORS 拦截；其接口条款同时要求使用可识别的 User-Agent，而浏览器无法自定义这个请求头。所以必须经服务端转发。

而 5EPlay 的接口 CORS 全开、且无需鉴权，所以前端可以直接调用——这也是它作为「第二数据源」在静态托管场景下的关键优势。
